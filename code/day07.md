...
【4】注意
	一旦使用scrapy发送POST请求，则必须要重写start_requests()方法，因为scrapy源码中的起始地址提交默认使用的是GET请求
```
...
使用redis_key实现分布式爬虫数据抓取

- **步骤**

  ```python
  【1】正常写scrapy的爬虫项目 - 非分布式
  【2】settings.py中设置相关变量 - 同分布式写法一
      2.1》SCHEDULER = 
      2.2》DUPEFILTER_CLASS = 
      2.3》SCHEDULER_PERSIST = 
      2.4》REDIS_HOST = 
      2.5》REDIS_PORT = 
  【3】改写爬虫文件,添加redis_key
      3.1》from scrapy_redis.spiders import RedisSpider
      3.2》class XxxSpider(RedisSpider):
      3.3》去掉start_urls
      3.4》redis_key = 'xxx:spider'
  【4】将scrapy项目拷贝到所有分布式爬虫服务器上,开始运行(发现所有爬虫都在等待,并未抓取)
  【5】启动爬虫
      redis-cli -h IP地址
      redis-cli>lpush xxx:spider 第一页要抓取的URL地址
          
  【发现】：所有等待的爬虫服务器都开始进行数据抓取
  ```
...
- **安装pytesseract**
  ...
  - **接口文档-ttapi.py**
    ...
    def base64_api(uname, pwd,  img):
        ...
        data = {"username": uname, "password": pwd, "image": b64, 'typid': 7}
